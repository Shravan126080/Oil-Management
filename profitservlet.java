package com.controller;

import java.io.IOException;
import java.sql.Connection;

import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import com.connection.connection;
import com.entity.storeentity;
import com.modules.pro_modules;

/**
 * Servlet implementation class profitservlet
 */
@WebServlet("/profitservlet")
public class profitservlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public profitservlet() {
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		response.getWriter().append("Served at: ").append(request.getContextPath());
	}

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		doGet(request, response);
		String stock=request.getParameter("pp");
		String profit=request.getParameter("pp1");
		
		storeentity p=new storeentity();
		p.setStock(stock);
		p.setProfit(profit);
		try {
			pro_modules ps=new pro_modules(connection.getConnection());
			boolean b=ps.insertdrop(p);
			if(b) {
				System.out.println("insert data");
				response.sendRedirect("fatchtable.jsp");
			}
		} catch (Exception e) {
			// TODO: handle exception
		}
		
				
		
	}

}